...
void draw() {
  ...
  case GAME_PAUSE:
    /*---------Print Text-------------*/
    printText("PAUSE", 40, 240);
    printText("Press ENTER to Resume", 20, 280);
    /*--------------------------------*/
    break;

  case GAME_WIN:
    /*---------Print Text-------------*/
    printText("WINNER", 40, 240);
    printText("SCORE" +":"+ point , 20, 280);
    /*--------------------------------*/
    winAnimate();
    break;
...
void checkAlienDead() {
  for (int i=0; i<bList.length-1; i++) {
    ...
    for (int j=0; j<aList.length-1; j++) {
      Alien alien = aList[j];
      if (bullet != null && alien != null   && !bullet.gone && !alien.die&& bList[i].bX + bList[i].bSize <= aList[j].aX + aList[j].aSize&& bList[i].bX - bList[i].bSize >= aList[j].aX - aList[j].aSize&& bList[i].bY + bList[i].bSize <= aList[j].aY + aList[j].aSize&& bList[i].bY - bList[i].bSize >= aList[j].aY - aList[j].aSize
...
void checkShipHit() {
  for (int i=0; i<lList.length-1; i++) {
    ...
    if (laser!= null && !laser.gone 
        && lList[i].lX + lList[i].lSize <= ship.posX + ship.shipSize
        && lList[i].lX - lList[i].lSize >= ship.posX - ship.shipSize
        && lList[i].lY + lList[i].lSize <= ship.posY + ship.shipSize
        && lList[i].lY + lList[i].lSize >= ship.posY - ship.shipSize
